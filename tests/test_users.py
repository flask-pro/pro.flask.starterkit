...
def test_users__crud(fx_app):
    print('\n--> test_users__crud')
    new_user = {'username': 'test_crud_user'}

    r_create = fx_app.post('/v1/users', json=new_user)
    assert r_create.status_code == 201
    assert 'id' in r_create.json
    assert r_create.json['username'] == new_user['username']

    r_get = fx_app.get(f'/v1/users/{r_create.json["id"]}')
    ...
    r_update = fx_app.put(f'/v1/users/{r_create.json["id"]}', json=updated_user)
    assert r_update.status_code == 200
    assert r_update.json['id'] == r_create.json['id']
    assert r_update.json['username'] == updated_user['username']

    r_delete = fx_app.delete(f'/v1/users/{r_create.json["id"]}')
...
def test_users__list(fx_app, fx_user, page, per_page, include_metadata):
    print('\n--> test_users__list')
    ...
    r_get = fx_app.get('/v1/users', query_string=query_string)
    assert r_get.status_code == 200
    assert r_get.json
...
def test_users__list__error_400(fx_app, fx_user, page, per_page, include_metadata):
